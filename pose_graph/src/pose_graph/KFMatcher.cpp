...
void KFMatcher::project_normal(Eigen::Vector2d kp, Eigen::Vector3d& point3d) const {
  const float invfx = 1.0f / params_.fx_;
  const float invfy = 1.0f / params_.fy_;

  const float u = kp[0];
  const float v = kp[1];
  point3d[0] = (u - params_.cx_) * invfx;
  point3d[1] = (v - params_.cy_) * invfy;
  point3d[2] = 1.0;
}
...
void KFMatcher::PnPRANSAC(const vector<cv::Point2f>& matched_2d_old,
  ...
  cv::Mat K = (cv::Mat_<double>(3, 3) << params_.fx_, 0, params_.cx_, 0, params_.fy_, params_.cy_, 0, 0, 1.0);
